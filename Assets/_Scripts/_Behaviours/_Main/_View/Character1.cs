...
namespace Main.View
{
	public class Character1 : SceneElement1
	{
		...
		public void SaveGallery()
		{
			...
			NativeGallery.SaveImageToGallery(skin, "3DSkinEditor", "Skin", null);
            if (Main.Controller.RatingController1.instance.Rate == 0)
            {
				Main.Controller.RatingController1.instance.rateCanvas.gameObject.SetActive(true);
		...
		private IEnumerator Load(string url, bool random)
		{
			...
			if (www.error == null)
			{
				...
				CapeController.Instance.maincharacter.GetComponent<CharacterMain>().LoadSkinDirect(tmpTexture);
			}
			else
			{
