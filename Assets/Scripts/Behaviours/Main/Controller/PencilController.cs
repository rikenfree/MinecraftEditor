...
{
	public class PencilController : SceneElement
	{
		private ColorController color;

		private RootController root;

		private Pixel lastPixelPainted;
		...
		private void Update()
		{
			if (!IsPointerOverUIObject())
			{
				...
				else if (Input.GetMouseButton(0)&& !root.camera.isAbleToRotate && root.AllowAction() && Physics.Raycast(ray, out hitInfo) && hitInfo.transform.tag == "Pixel")
				{
					Pixel component = hitInfo.transform.GetComponent<Pixel>();
					SmartPaint(component);
				}
			}
		}

		private void SmartPaint(Pixel pixel)
		{
