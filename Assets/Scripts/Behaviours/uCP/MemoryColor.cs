...
namespace uCP
{
	public class MemoryColor : MonoBehaviour
	{
		...
		[NonSerialized]
		public List<GradationUI> Memories = new List<GradationUI>();

		[NonSerialized]
		public GradationUI currentMemory;
		...
		public virtual void CreateNewButton(Color color)
		{
			if (Memories.Count < MaxMemory)
			{
				...
				GradationUI grad = gameObject.GetComponent<GradationUI>();
				Memories.Add(grad);
		...
		{
			return Memories.ConvertAll((GradationUI x) => x.colors[1]).ToArray();
		}
