...
public class StoreController : MonoBehaviour, IStoreListener
{
    ...
    ///public UIStoreWindow storeWindow;

    bool hasInitialized;

    //public static System.Action<ProductList> onStoreLoaded;

    //  [SerializeField] ProductList productList;
    [Header("Consumable")]
    public List<string> Consumable_ProductIdList = new List<string>();

    [Header("Non-Consumable")]
    public List<string> NonConsumable_ProductIdList = new List<string>();

    [Header("Subscription")]
    public List<string> Subscription_ProductIdList = new List<string>();
    [SerializeField] List<string> transactions = new List<string>();

    public TextAsset debug_productsJSON;
    ...
    void Initialize_Store()
    {
        ...
        if (Consumable_ProductIdList.Count > 0)
        {
            for (int i = 0; i < Consumable_ProductIdList.Count; i++)
            {
                builder.AddProduct(Consumable_ProductIdList[i], ProductType.Consumable, new IDs
                {
                    { Consumable_ProductIdList[i], GooglePlay.Name },
                    { "com.mygame.100goldcoins.ios", AppleAppStore.Name }
                });
            }
        }
        if (NonConsumable_ProductIdList.Count > 0)
        {
            for (int i = 0; i < NonConsumable_ProductIdList.Count; i++)
            {
                builder.AddProduct(NonConsumable_ProductIdList[i], ProductType.NonConsumable, new IDs
                {
                    { NonConsumable_ProductIdList[i], GooglePlay.Name },
                    { "com.mygame.100goldcoins.ios", AppleAppStore.Name }
                });
            }
        }
        if (Subscription_ProductIdList.Count > 0)
        {

            for (int i = 0; i < Subscription_ProductIdList.Count; i++)
            {
                builder.AddProduct(Subscription_ProductIdList[i], ProductType.Subscription, new IDs
                {
                    { Subscription_ProductIdList[i], GooglePlay.Name },
                    { "com.mygame.100goldcoins.ios", AppleAppStore.Name }
                });
            }
        }
    ...
    public void OnInitialized(IStoreController controller, IExtensionProvider extensions)
    {
        ...
        string msg = "STORE INITIALIZED >> Available items:";
        foreach (var item in controller.products.all)
        {
            ...
            if (item.hasReceipt)
                transactions.Add(item.transactionID);
        }
        ...
        //onStoreLoaded?.Invoke(productList);
    }
    ...
    public Product Get_Product(string _productID)
    {
        return m_Controller.products.WithID(_productID);
    }
    public string GetPrice(string productID)
    {
        if (m_Controller == null) Initialize_Store();
        Debug.Log("Prize:" + m_Controller.products.WithID(productID).metadata.localizedPrice.ToString());
        return m_Controller.products.WithID(productID).metadata.localizedPriceString;
    }
    ...
    public string Get_Transactions()
    {
        ...
        for (int iTransaction = 0; iTransaction < transactions.Count; iTransaction++)
        {
            msg += transactions[iTransaction] + ",";
            //msg += "\nTransaction [" + iTransaction + "]: " + transactions[ iTransaction ];
        }
    ...
    public void RemoveAds()
    {
        if(SuperStarAd.Instance.NoAds == 1)
        {
            ...
            AdmobManager.Instance.isFirstOpenAdd = false;
        }
    }

    public bool HasInitialized { get { return hasInitialized; } }

    // public ProductList ProductList { get => productList; }
}
